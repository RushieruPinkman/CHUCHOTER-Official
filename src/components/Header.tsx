    ...
    <>
      <header
        className={`fixed inset-x-0 top-0 z-50 transition-colors duration-300 ${
          scrolled
            ? "border-b border-[var(--color-border)] bg-deep/95"
            : "bg-transparent"
        }`}
      >
        <div className="site-container flex h-[72px] items-center justify-between md:h-20">
          <Link
            href="/"
            ...
            {NAV_ITEMS.map((item, index) => {
              ...
              return (
                <span key={item.href} className="flex items-center">
                  ...
                  <Link
                    href={item.href}
                    className={`group px-4 py-2 text-center transition-colors duration-300 ${
          ...
          <button
            type="button"
            className="relative z-10 flex h-10 w-10 flex-col items-center justify-center gap-[5px] border border-[var(--color-border)] lg:hidden"
