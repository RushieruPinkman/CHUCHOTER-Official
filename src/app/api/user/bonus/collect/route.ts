...
import { collectBonusRoulette } from "@/lib/bonus-roulette-store";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await collectBonusRoulette(user.userKey, type);
    return NextResponse.json(result);
  } catch (error) {
    return storageErrorResponse(error, "ボーナスの受け取りに失敗しました");
  }
}
